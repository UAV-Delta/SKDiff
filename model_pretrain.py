...
class MyLoss_Pretrain(torch.nn.Module):
    ...
    def forward(self, pred, tar):     # 交叉熵损失，给定头实体和关系，预测尾实体
        kg_pred_max = torch.max(pred, dim=1)[0].view(-1, 1)
        kg_pred_log_max_sum = torch.log(torch.sum(torch.exp(pred-kg_pred_max), dim=1)).view(-1, 1) #防止值过大
        kg_pred_log_softmax = pred - kg_pred_max - kg_pred_log_max_sum  
        loss_kge = - kg_pred_log_softmax[tar==True].mean()   #对batch样本求平均
        return loss_kge
